    ...
    public override void DoSettingsWindowContents(Rect rect)
    {
        ...
        if (ModLister.HasActiveModWithName("Simple sidearms"))
        {
            listing_Standard.Label(
                $"NOTICE: Simple Sidearms-mod is loaded. Will only be using setting {findingSettings[0]} regardless of active setting below.");
        }
        ...
        listing_Standard.Gap();
        listing_Standard.Label(
            $"{findingSettings[0]}: Pawns will automatically find an appropriate weapon if they do not have one equipped.");
        listing_Standard.Label(
            $"{findingSettings[1]}: Same as {findingSettings[0]} with the addition that Brawlers and Hunters will replace their equipped weapon if needed.");
        listing_Standard.Label(
            $"{findingSettings[2]}: All pawns will automatically replace their weapon for a better one if possible.");
        listing_Standard.Gap();
        listing_Standard.Label(
            "Weapon priority is based on weapon-type, value, damage-type and pawn skills/traits.");
        listing_Standard.Label(
            "Outfits now contain weapons as well and can be used for filtering allowed weapon types.");
